...
public class ProductController {
    ...
    public ResponseEntity<List<ProductDTO>> addProductsBulk(@RequestBody List<BulkProductItem> items){
        List<ProductDTO> createdProducts = productService.addProductsBulk(items);
        return new ResponseEntity<>(createdProducts, HttpStatus.CREATED);
    }

    @PostMapping("/admin/products/reindex")
    public ResponseEntity<APIResponse> reindexProducts(){
        long count = productService.reindexAllProducts();
        APIResponse response = new APIResponse("Re-indexed " + count + " products into the vector store", true);
    ...
    @GetMapping("/public/products/semantic-search")
    public ResponseEntity<ProductResponse> semanticSearch(
            @RequestParam(name = "query") String query,
            @RequestParam(name = "topK", defaultValue = "5", required = false) Integer topK,
            @RequestParam(name = "minPrice", required = false) Double minPrice,
            @RequestParam(name = "maxPrice", required = false) Double maxPrice){
        ProductResponse productResponse = productService.semanticSearch(query, topK, minPrice, maxPrice);
        return new ResponseEntity<>(productResponse, HttpStatus.OK);
    }
